 ...
 * Returns a promise-based signal with output modifiers.
 *
 ...
 * @returns {SignalNode<Promise> & {refresh: function(): void, debounce: function(number), throttle: function(number), unpack: function()}}
 *   A promise-based signal with refresh() method and output modifiers
 */
computed.fetcher = (url, options = {}) => {
	...
	// Use computed.async to create the promise-based signal
	const fetcherSignal = computed(async (signal) => {
		// Subscribe to refresh trigger to force re-fetch
		refreshTrigger.v;
		...
		const fetchOptions = isCustomFetcher ? optionsValue : { ...optionsValue, signal };

		return isCustomFetcher
			? fetcher(fetchOptions)
			: fetcher(urlValue, fetchOptions);
	});

	// Add refresh method and output modifiers
	const resultProxy = new Proxy(fetcherSignal, {
		get(target, prop) {
			...
			// OUTPUT modifier: Debounce the output (the promise signal updates)
			if (prop === 'debounce') {
				return (delay) => {
					const debouncedSignal = computed.debounceInput(() => target.v, delay);
					return addOutputModifiers(debouncedSignal);
				};
			}

			// OUTPUT modifier: Throttle the output (the promise signal updates)
			if (prop === 'throttle') {
				return (delay) => {
					const throttledSignal = computed.throttleInput(() => target.v, delay);
					return addOutputModifiers(throttledSignal);
				};
			}
...
/**
 * INPUT modifier: Debounces the input signals (url/options changes) before fetching
 * Usage: computed.debounceInput(300).fetcher(url)
 * @param {number|Function} delayOrFn - Delay in milliseconds or function to compute
 * @param {number} [delay] - Delay if first arg is a function
 * @param {Function} [set] - Optional setter function
 * @returns {Object|SignalNode} Builder object or debounced computed signal
 */
computed.debounceInput = function(delayOrFn, delay, set) {
	// Old API: computed.debounceInput(getFn, delay)
	if (typeof delayOrFn === 'function') {
		return createDeferredComputed(delayOrFn, (fn) => effect.debounced(fn, delay), set);
	}

	// New API: computed.debounceInput(delay) returns a builder
	const delayMs = delayOrFn;

	return {
		/**
		 * Creates a fetcher with debounced inputs (returns promise with modifiers)
		 */
		fetcher: (url, options = {}) => {
			// Create a debounced trigger signal
			const debouncedTrigger = signal(0);
			let timeoutId;

			// Watch the url/options and debounce changes
			effect(() => {
				isSignal(url) ? url.v : url;
				isSignal(options) ? options.v : options;

				clearTimeout(timeoutId);
				timeoutId = setTimeout(() => {
					debouncedTrigger.v = debouncedTrigger.v + 1;
				}, delayMs);
			});

			// Create the fetcher that watches the debounced trigger
			const isCustomFetcher = typeof options === 'function';
			const fetcher = isCustomFetcher ? options : defaultFetcher;

			const fetcherSignal = computed(async (signal) => {
				// Subscribe to debounced trigger
				debouncedTrigger.v;
			...
			return addOutputModifiers(fetcherSignal);
		}
	};
};
...
/**
 * INPUT modifier: Throttles the input signals (url/options changes) before fetching
 * Usage: computed.throttleInput(500).fetcher(url)
 * @param {number|Function} delayOrFn - Delay in milliseconds or function to compute
 * @param {number} [delay] - Delay if first arg is a function
 * @param {Function} [set] - Optional setter function
 * @returns {Object|SignalNode} Builder object or throttled computed signal
 */
computed.throttleInput = function(delayOrFn, delay, set) {
	// Old API: computed.throttleInput(getFn, delay)
	if (typeof delayOrFn === 'function') {
		return createDeferredComputed(delayOrFn, (fn) => effect.throttled(fn, delay), set);
	}

	// New API: computed.throttleInput(delay) returns a builder
	const delayMs = delayOrFn;

	return {
		/**
		 * Creates a fetcher with throttled inputs (returns promise with modifiers)
		 */
		fetcher: (url, options = {}) => {
			// Create a throttled trigger signal
			const throttledTrigger = signal(0);
			let intervalId;
			let timeoutId;

			// Watch the url/options and throttle changes
			effect(() => {
				isSignal(url) ? url.v : url;
				isSignal(options) ? options.v : options;
				...
				if (!intervalId) {
					...
					throttledTrigger.v = throttledTrigger.v + 1;
			...
			// Create the fetcher that watches the throttled trigger
			const isCustomFetcher = typeof options === 'function';
			const fetcher = isCustomFetcher ? options : defaultFetcher;

			const fetcherSignal = computed(async (signal) => {
				// Subscribe to throttled trigger
				throttledTrigger.v;
...
/**
 * Helper function that adds output modifiers (.debounce, .throttle, .unpack) to a promise signal
 * @param {SignalNode} promiseSignal - A signal that holds promises
 * @returns {SignalNode} The signal with added modifier methods
 */
function addOutputModifiers(promiseSignal) {
	return new Proxy(promiseSignal, {
		get(target, prop) {
			// OUTPUT modifier: Debounce the output (the promise signal updates)
			if (prop === 'debounce') {
				return (delay) => {
